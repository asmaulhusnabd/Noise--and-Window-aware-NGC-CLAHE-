...
# --- preview: Original, Synthetic, NGC-CLAHE, Proposed ---
...
for r in sorted(p_ref.iterdir()):
    ...
    stem = r.stem
    try:
        real01 = load_real_windowed(r)                         # original (windowed)
        synth  = np.load(p_synth / f"{stem}.npy")              # synthetic degraded
        base   = np.load(p_out / f"{stem}_ngcclahe.npy")       # NGC-CLAHE
        prop   = np.load(p_out / f"{stem}_proposed.npy")       # NW-GC-CLAHE
    except Exception as e:
        print(f"skip {r.name}: {e}")
        continue

    fig, ax = plt.subplots(1, 4, figsize=(16, 4))
    ax[0].imshow(real01, cmap='gray'); ax[0].set_title(f"Original\n(WL/WW {BRAIN_WL}/{BRAIN_WW})")
    ax[1].imshow(synth,  cmap='gray'); ax[1].set_title("Synthetic\n(low contrast)")
    ax[2].imshow(base,   cmap='gray'); ax[2].set_title("NGC-CLAHE")
